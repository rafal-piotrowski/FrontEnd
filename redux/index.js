...
import forms from './src/redux/reducers/forms.js';

// konektor służący podłączaniu się do store-a
// import { connect } from 'pwa-helpers/connect-mixin.js';

import { updatePage } from './src/redux/actions/forms.js';
  ...
  render() {
    return html`
      ...
      <vaadin-tabs class="${this.smallScreen ? 'nav' : ''}" orientation="${this.smallScreen ? 'vertical' : 'horizontal'}" selected=${this.tabs.indexOf(this.activeTab)} theme="${this.smallScreen ? '' : 'centered'}">
        ...
        <vaadin-tab @click=${() => { this.switchRoute('first'); store.dispatch(updatePage(1)); }}>formatka #1</vaadin-tab>
        <vaadin-tab @click=${() => { this.switchRoute('second'); store.dispatch(updatePage(2)); }}>formatka #2</vaadin-tab>
        <vaadin-tab @click=${() => { this.switchRoute('third'); store.dispatch(updatePage(3)); }}>formatka #3</vaadin-tab>
      </vaadin-tabs>
